...
- ASK FIRST, ALWAYS (Jordan, 7 Sept): before ANY outbound message leaves, to anyone, by any channel, show it and ask "can I send it out". Wait for a yes. This replaces every looser reading of the approval rule. "Send it if it works", "go ahead" earlier in the thread, or approval of an earlier draft is NOT approval of the message actually about to leave. A scheduled job that sends on its own is not exempt: it must ask on the turn it fires.
