...
export default function PartnerConnectCalendar() {
  ...
  useEffect(() => {
    if (connected) {
      toast.success(t("partner.connectCalendar.toastConnected"));
      loadPartner();
    }
    if (error) {
      toast.error(t("partner.connectCalendar.toastConnectionFailed", { error }));
    }
  }, [connected, error]);
  ...
  const handleConnect = () => {
    ...
    if (clientId.startsWith("PASTE_")) {
      toast.error(t("partner.connectCalendar.toastNotSetUp"));
      return;
    }
    setConnecting(true);
  ...
  const handleDisconnect = async () => {
    if (!partner) return;
    if (!confirm(t("partner.connectCalendar.confirmDisconnect"))) return;
    ...
    if (error) {
      toast.error(t("partner.connectCalendar.toastDisconnectFailed"));
    } else {
      toast.success(t("partner.connectCalendar.toastDisconnected"));
      loadPartner();
    }
  };
  ...
  const handleAutoConfirm = async (enabled: boolean) => {
    ...
    if (error) {
      toast.error(t("partner.connectCalendar.toastUpdateFailed"));
    } else {
      toast.success(enabled ? t("partner.connectCalendar.toastAutoConfirmOn") : t("partner.connectCalendar.toastAutoConfirmOff"));
      loadPartner();
    }
  };
      ...
      <div className="px-6 py-5 border-b border-border bg-card">
        <div className="max-w-xl mx-auto">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-xs text-muted-foreground">{t("partner.connectCalendar.stepLabel")}</p>
              <h1 className="font-display text-lg font-bold">{t("partner.connectCalendar.title")}</h1>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-xl mx-auto px-6 py-6 space-y-5">
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                <h2 className="font-display font-bold text-base">
                  {isConnected ? t("partner.connectCalendar.connectedTitle") : t("partner.connectCalendar.notConnectedTitle")}
                </h2>
                <p className="text-sm text-muted-foreground mt-1">
                  {isConnected
                    ? t("partner.connectCalendar.connectedDesc", { calendar: partner.google_calendar_id || t("partner.connectCalendar.yourGoogleCalendar") })
                    : t("partner.connectCalendar.notConnectedDesc")}
                </p>
              </div>
            </div>
          </CardContent>
        ...
        {error && (
          <Card className="border-red-500/50 bg-red-50/50">
            <CardContent className="p-4 flex items-center gap-3">
              ...
              <div>
                <p className="text-sm font-semibold text-red-600">{t("partner.connectCalendar.connectionFailedTitle")}</p>
            ...
            >
              {connecting ? (
                ...
                <>
                  ...
                  {t("partner.connectCalendar.connectButton")}
                </>
              )}
            </Button>
            <p className="text-xs text-center text-muted-foreground">
              {t("partner.connectCalendar.connectHint")}
            </p>
          </div>
        ) : (
          <div className="space-y-3">
            ...
            >
              {disconnecting ? <Loader2 className="h-4 w-4 animate-spin" /> : <><Trash2 className="h-4 w-4 mr-2" /> {t("partner.connectCalendar.disconnectButton")}</>}
            </Button>
          </div>
        )}
        ...
        <Card className="bg-card border-border">
          <CardContent className="p-5 space-y-4">
            <h3 className="font-display font-bold text-sm">{t("partner.connectCalendar.howItWorks")}</h3>

            <div className="space-y-3">
              {[
                {
                  step: "1",
                  title: t("partner.connectCalendar.step1Title"),
                  desc: t("partner.connectCalendar.step1Desc"),
                  done: isConnected,
                },
                {
                  step: "2",
                  title: t("partner.connectCalendar.step2Title"),
                  desc: t("partner.connectCalendar.step2Desc"),
                  done: isConnected,
                },
                {
                  step: "3",
                  title: t("partner.connectCalendar.step3Title"),
                  desc: t("partner.connectCalendar.step3Desc"),
                  done: isConnected,
                },
                {
                  step: "4",
                  title: t("partner.connectCalendar.step4Title"),
                  desc: t("partner.connectCalendar.step4Desc"),
                  done: isConnected,
                },
        ...
        {isConnected && (
          <Card className="bg-card border-border">
            <CardContent className="p-5 space-y-4">
              <div>
                <h3 className="font-display font-bold text-sm mb-1">{t("partner.connectCalendar.autoConfirmTitle")}</h3>
                <p className="text-xs text-muted-foreground">
                  {t("partner.connectCalendar.autoConfirmDesc")}
                </p>
              </div>
              ...
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-semibold">
                    {partner?.auto_confirm_bookings ? t("partner.connectCalendar.autoConfirmOnLabel") : t("partner.connectCalendar.autoConfirmOffLabel")}
                  </p>
                </div>
                <button
        ...
        <div className="text-center">
          ...
          >
            {t("partner.connectCalendar.manualAvailabilityLink")}
          </button>
        </div>
      </div>
    </div>
